    ...
    <section className={`relative mx-auto min-h-screen w-full overflow-clip`}>
      <div className={`${styles.paddingX} absolute inset-0 top-[124px] z-10 mx-auto flex h-fit flex-col items-start xs:top-[120px]`}>
        <div className='mx-auto w-full text-center'>
          <h2 className={`${styles.heroHeadText} text-center text-white`}>
            ...
            <div className='relative h-[33px] overflow-clip xs:h-[42px] sm:h-[51px] md:h-[60px] lg:h-[72px] xl:h-[75px] 2xl:h-[84px]'>
              <div className='text-animate absolute flex w-full flex-col gap-y-3'>
                <span className={`${styles.heroHeadText} animate-line relative z-0 mx-auto w-fit text-white`}>{heroHeadings[heroHeadings.length - 1]}</span>
                {heroHeadings.map((text) => (
                  <span
                    className={`${styles.heroHeadText} animate-line relative z-0 mx-auto w-fit text-white`}
                    key={text}
                  >
                    {text}
                  </span>
          ...
          <p className={`${styles.heroParagraphText} mx-auto mt-8 max-w-[348px] text-center italic text-white sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl`}>Kami tunggu para petualang hebat di sekolah kami, together to be excellent!</p>
        </div>
