...
const DetailNewsItem = ({ index, subHeading, body, img }) => (
  <>
    {subHeading && <h5 className={`${styles.heroSubText} font-bold text-sky-500`}>{subHeading}</h5>}
    {body && <p className='leading-[200%]'>{body}</p>}
    {img.url && (
        ...
        <article className='flex flex-col gap-16 sm:p-4 lg:pl-12'>
          <section className='flex flex-col items-start lg:flex-row'>
            ...
            <div className='lg:basis-7/12 xl:basis-8/12'>
              <p className='w-full max-w-2xl text-sm text-slate-500 max-lg:mt-6'>
                Published {date} in <br className='sm:hidden' /> <Link className='underline'>Al-Izzah News</Link>
              </p>
              <h2 className='mb-12 mt-8 text-2xl/normal font-bold text-black-200 sm:text-3xl/normal lg:text-4xl/normal 2xl:text-5xl/normal'>{title}</h2>
          ...
          <section className='flex flex-col items-start lg:flex-row'>
            <div className='flex flex-col gap-4 lg:basis-3/12 xl:basis-2/12'>
              <span className='text-xs uppercase text-slate-400'>Share this Post</span>
              <ul className='flex gap-1'>
                {footerIcons.map((icon, index) => (
                  ...
                  >
                    <a
                      href={`${icon.url}`}
                      className='flex p-1 hover:bg-slate-200 rounded-[9px] transition duration-300'
                      target='_blank'
                    >
                      <div className='p-px bg-slate-200 rounded-[7px]'>
                        <img
                          src={`${icon.icon}`}
                          alt=''
                          height='24'
...
//   content.subHeading && (
//     <h5
//       className='text-lg font-bold leading-[150%] sm:text-xl lg:text-2xl 2xl:text-3xl'
//       key={index}
//     >
//       {content.subHeading}
//     </h5>
